...
DEFAULT_KEEP = (
    "embed,wte,modulation,adaln,ada_ln,time_,guidance,"
...
VOCAB_DIM = (
    50000  # FFN dims top out ~36k in these models; >=50k => embedding/vocab table
)
...
def verify(input_path, output_path, prefix, layers, device, n, fmt, layout):
    ...
    errs = []
    with (
        safe_open(input_path, framework="pt", device="cpu") as fsrc,
        safe_open(output_path, framework="pt", device="cpu") as fout,
    ):
        out_keys = set(fout.keys())
        for layer in tqdm(
            sample, desc="verifying", unit="layer", disable=not _HAVE_TQDM
        ):
            ...
            if wk not in out_keys or sk not in out_keys:
                ...
                continue
            qdata = fout.get_tensor(wk).to(
                device
            )  # mxfp8: fp8 (maybe padded); nvfp4: uint4-packed uint8
            ...
            if fmt == "nvfp4":
                ...
                ts = fout.get_tensor(f"{layer}.weight_scale_2").to(
                    device
                )  # weight_scale_2: f32 global
                ...
                params = layout_cls.Params(
                    scale=bs, orig_dtype=torch.bfloat16, orig_shape=tuple(src.shape)
                )
            deq = QuantizedTensor(qdata, layout, params).dequantize().float()
            errs.append(
                (
                    (
                        (deq - src).abs().mean() / src.abs().mean().clamp_min(1e-8)
                    ).item(),
                    layer,
                )
            )
    ...
    print(
        f"  reload relerr  mean={mean:.4f}  median={e[len(e)//2]:.4f}  max={e[-1]:.4f}"
    )
    print("  worst layers:")
    ...
    if e[-1] > 0.10:
        print(
            "  WARNING: max reload error >0.10 - investigate save/load, not just recipe."
        )
    else:
        print(
            "  OK: reload matches in-memory error range -> on-disk format is correct."
        )
    return {
        "sampled": len(sample),
        "mean": mean,
...
def write_report(
    ...
    *,
    ...
    fmt_desc = {
        "mxfp8": "block-32 microscaling FP8 (E4M3 + E8M0 block scales)",
        "nvfp4": "4-bit NVFP4 (E2M1 + group-16 FP8 block scale + per-tensor FP32 scale)",
    }[fmt]
    L = []
    L.append(f"# Quantization report — `{os.path.basename(args.output)}`\n")
    L.append(
        f"- **Source:** `{os.path.basename(args.input)}` — {in_size:.2f} GB (bf16)"
    )
    L.append(
        f"- **Output:** `{os.path.basename(args.output)}` — {out_size:.2f} GB "
        f"({100*out_size/in_size:.0f}% of source)"
    )
    L.append(
        f"- **Format:** {fmt} — {fmt_desc}; ComfyUI-native (`comfy_quant` metadata, no custom nodes)"
    )
    ...
    L.append(
        f"- **Generated:** {datetime.date.today().isoformat()} by `quantize_model_blackwell.py`"
    )
    L.append(
        f"- **Command:** `{' '.join(os.path.basename(a) if i == 0 else a for i, a in enumerate(sys.argv))}`\n"
    )
    ...
    L.append(
        f"- **Diffusion prefix:** `{prefix!r}`"
        + ("" if not dropped_ns else f" — dropped (not in output): {dropped_ns}")
    )
    L.append(
        f"- **Selection rule:** quantize 2D Linear `.weight` with min(dim) ≥ {args.min_dim}, "
        "except names in the keep-list and vocab/embedding tables."
    )
    keep_eff = "DEFAULT_KEEP" + (
        " + NVFP4_KEEP_EXTRA" if fmt == "nvfp4" and args.keep_bf16 is None else ""
    )
    L.append(
        f"- **keep-bf16 patterns** ({keep_eff}): "
    ...
    if fmt == "nvfp4" and calib:
        L.append(
            f"- **Calibration:** `input_scale` applied to {n_input_scale}/{n_q} quantized layers "
    ...
    for reason in sorted(rep_k, key=lambda r: -sum(c[0] for c in rep_k[r].values())):
        ...
        L.append(
            _table(sorted(((p, c[0], c[1]) for p, c in d.items()), key=lambda r: r[0]))
            + "\n"
        )
    ...
    if verify_stats:
        v = verify_stats
        floor = (
            "~0.02 is near the mxfp8 representational floor"
            if fmt == "mxfp8"
            else "~0.09 is expected for 4-bit nvfp4"
        )
        L.append("## Quality — reload verification\n")
        L.append(
            f"Round-trip dequantization error over {v['sampled']} sampled quantized layers, "
            "reconstructed exactly as ComfyUI's loader does (so it doubles as a load check):\n"
        )
        L.append(
            f"- relative L1 error: **mean {v['mean']:.4f}, median {v['median']:.4f}, "
            f"max {v['max']:.4f}** ({floor})."
        )
        L.append(
            "- worst layers: "
            + ", ".join(f"`{lay}` {r:.4f}" for r, lay in v["worst"])
            + "\n"
        )
    ...
    L.append(
        f"Load with {loader}; the `comfy_quant` metadata is auto-detected and the weights stay "
...
def _avail_ram_gb():
    """Read available RAM from /proc/meminfo. Returns GB or None on failure."""
    try:
...
def save_with_progress(out_sd, path, metadata):
    """Save the safetensors with a two-phase progress bar.

    safetensors serializes all tensors into one in-memory buffer (Rust, opaque to Python)
    before touching the output file. Phase 1 tracks RAM consumption during that gap;
    phase 2 tracks file growth during the actual disk write.
    """
    if not _HAVE_TQDM:
    ...
    def monitor():
        ...
        while not stop.wait(0.25):
            ...
            if cur > 0 and not writing:
                bar.set_description("writing → disk")
                bar.set_postfix_str("")
                writing = True
            if cur > last:
                bar.update(cur - last)
                last = cur
            elif not writing and ram0 is not None:
                ram_now = _avail_ram_gb()
                if ram_now is not None:
                    bar.set_postfix_str(f"+{ram0 - ram_now:.1f} GB", refresh=False)
...
def main():
    ...
    if args.output is None:
        args.output = args.input.replace(
            ".safetensors", f"_{args.format.lower()}.safetensors"
        )
    assert (
        args.format in QUANT_ALGOS
    ), f"comfy_kitchen lacks {args.format} support — update it"
    ...
    if args.keep_bf16 is not None:
        ...
        if fmt == "nvfp4":
            keep_src += (
                "," + NVFP4_KEEP_EXTRA
            )  # keep per-block modulation bf16 at 4-bit
    ...
    with safe_open(args.input, framework="pt", device="cpu") as f:
        ...
        print(
            f"keep-bf16 patterns: {keep_patterns}{'  + force-quant '+str(force_quant) if force_quant else ''}"
        )
        ...
        if not kept:
            sys.exit(
                "ERROR: no keys under the diffusion prefix — pass --diffusion-prefix explicitly."
            )
        ...
        pbar = tqdm(
            kept, desc=f"quantizing -> {fmt}", unit="layer", disable=args.dry_run
        )
        for k in pbar:
            t = f.get_tensor(k)
            ...
            if action == "quant":
                ...
                qt = QuantizedTensor.from_float(
                    w, layout
                )  # nvfp4 recalculates its global scale
                ...
                if fmt == "nvfp4" and layer in calib:  # calibrated activation scale
                    out_sd[f"{layer}.input_scale"] = (
                        (calib[layer] / NVFP4_SCALE_DENOM).float().cpu()
                    )
                    n_input_scale += 1
                if args.check_error:
                    err = (
                        (qt.dequantize().float() - w.float()).abs().mean()
                        / w.float().abs().mean().clamp_min(1e-8)
                    ).item()
                ...
                if reason.startswith(("sensitive", "vocab")):
                    ...
                    if args.dry_run:
                        print(
                            f"  KEEP   {ok[:-len('.weight')]:64s} {tuple(t.shape)} ({reason})"
                        )
                if not args.dry_run:
    ...
    if calib:
        print(
            f"input_scale: matched {n_input_scale}/{n_q} quantized layers "
            f"(calib file had {len(calib)} entries)"
        )
        if n_input_scale == 0:
            print(
                "  WARNING: no calib names matched quantizer layer names — the calibration node's "
                "module names differ from the quantizer keys; input_scale was NOT applied."
            )
        elif n_input_scale < n_q:
            missing = n_q - n_input_scale
            print(
                f"  note: {missing} quantized layers had no calib entry (those use dynamic "
    ...
    meta = {
        k: v
        for k, v in src_meta.items()  # carry over non-quant metadata
    ...
    meta["quantization_summary"] = json.dumps(  # self-documenting (visible on HF)
        {
            "tool": "quantize_model_blackwell.py",
            "format": fmt,
            "quantized_layers": n_q,
            "bf16_layers": n_pass,
            "quantized_params": q_params,
            "bf16_params": bf_params,
            "min_dim": args.min_dim,
            "diffusion_prefix": prefix,
        }
    )
    ...
    if args.report is not None:
        report_path = args.report or (
            os.path.splitext(args.output)[0] + ".quant_report.md"
        )
        write_report(
            report_path,
            args=args,
